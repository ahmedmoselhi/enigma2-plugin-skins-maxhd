...
#    Coded by shamann (c)2010 mao by Schomi 2011
#
...
class SchoWatches(Renderer):
	...
	def draw_line(self, x0, y0, x1, y1):
		...
		for x in range(x0, x1 + 1):
			if steep:
				self.instance.fillRect(eRect(y, x, 6, 6), self.fColor)
			else:          
				self.instance.fillRect(eRect(x, y, 6, 6), self.fColor)
			error = error + deltay
			if error > 0:
				y = y + ystep
				error = error - deltax
