...
class Bitrate2(Converter, object):
	...
	def initBitrateCalc(self):
		...
		if vpid > 0 and (self.type is self.VPID or (self.type is self.FORMAT and "%V" in self.sfmt)):
		...
		if apid > 0 and (self.type is self.APID or (self.type is self.FORMAT and "%A" in self.sfmt)):
	...
	def getText(self):
		if self.type is self.VPID:
			...
			while True:
				...
				if f is 'V':	# %V - VideoBitrate
					ret += "%d"%(self.video)
				elif f is 'A':	# %A - AudioBitrate
					ret += "%d"%(self.audio)
				else:
					ret += f
				if pos+1 >= l: break
